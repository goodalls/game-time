...
module.exports = class Projectile extends GamePiece {
  ...
  draw(context) {
    this.bullets.forEach(function (bullet, index, array) {
      ...
      // trying to shoot either a 0 or 1, code below continues to
      // excute as the bullet travels making it flash back and forth
      // need to find a way to pick one randomly and make it persists  
      
      // let bulletMagazine = [0,1];
      // let binaryBullet = bulletMagazine[Math.floor(Math.random()
      //     * bulletMagazine.length)];

      // context.font = '12px Aldrich';
      // context.fillText(`${binaryBullet}`, bullet.x, bullet.y);
